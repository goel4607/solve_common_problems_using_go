...
func (s *Soln1WithoutMemoization) findMinCostOfClimbingStairs(cost []int) (int, int) {
	n := len(cost)

	return int(math.Min(float64(s.findMinCost(cost, n-1)), float64(s.findMinCost(cost, n-2)))), s.numCalls
