...
func (s Prac23MayTry1) findMinCostOfClimbingStairs(cost []int) (int, int) {
...
func (s Prac23MayTry1) minCost(cost []int, minCostVals []int, i, numCalls int) (int, int) {
	if i <= 1 {
		return cost[i], numCalls
	}

	if i < len(cost) && minCostVals[i] != -1 {
		return minCostVals[i], numCalls
	}

	var prev, nc1 int
	if minCostVals[i-1] == -1 {
		prev, nc1 = s.minCost(cost, minCostVals, i-1, numCalls+1)
	...
	if minCostVals[i-2] == -1 {
		nxtToPrev, nc2 = s.minCost(cost, minCostVals, i-2, numCalls+1)
