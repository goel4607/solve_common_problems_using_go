...
func TestCountHeight(t *testing.T) {
	...
	for i, tt := range tests {
		...
		if assert.Equal(t, tt.expectedSortedOpt, actual) {
			t.Logf("\t%s\texpected sorted output=%v, actual=%v", solve_common_problems_using_go.Passed, tt.expectedSortedOpt, actual)
		} else {
			t.Logf("\t%s\txpected sorted output=%v, actual=%v", solve_common_problems_using_go.Failed, tt.expectedSortedOpt, actual)
		}
	}
}
